...
`extended_params`
This field allows a parameter to have a list of values. Currently, unsupported.
...
`oauth_keys`
Defined as a `map<string, string>` instance, that maps an OAuth client ID with a corresponding access token value. Currently unsupported and under development.
...
`certificate`
The client certificate used to authenticate the request. Exists in the session instance if mTLS is configured for the API. Currently unsupported.

`max_query_depth`
For graphQL APIs, if the session key has a maximum query depth limit defined then it is included in the session instance. Currently unsupported and under development.
