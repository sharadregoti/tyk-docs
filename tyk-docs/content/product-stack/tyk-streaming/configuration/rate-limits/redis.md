...
description: Explains an overview of rate limiting using Redis
tags: [ "Tyk Streams", "Redis Rate Limits" ]
---

A rate limit implementation using Redis that works by using a simple token bucket algorithm to limit the number of requests to a given count within a given time period. The rate limit is shared across all instances of Tyk Streams that use the same Redis instance, which must all have a consistent count and interval.
