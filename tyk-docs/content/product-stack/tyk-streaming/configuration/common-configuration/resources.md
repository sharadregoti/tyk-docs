...
Then when you execute Tyk Streams, use the environment variable to choose your resource: `FEATURE_REQUEST=get_foo tyk-streams -c ./your_config.yaml`.

### With Imports

However, Tyk Streams allows you to import resources from separate files with the cli flag `-r` or `-resources`, which can be a useful way to switch out resources with common names based on your chosen environment. For example, with a main configuration file `config.yaml`:
...
```sh
tyk-streams -r ./staging/request.yaml -c ./config.yaml
```

Or:

```sh
tyk-streams -r ./production/request.yaml -c ./config.yaml
```

These flags also support wildcards, which allows you to import an entire directory of resource files like `tyk-streams -r "./staging/*.yaml" -c ./config.yaml`. -->
